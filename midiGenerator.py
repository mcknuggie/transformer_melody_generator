# Inference class - responsible for generating a melody

"""
This script defines the MelodyGenerator class, whih is responsible for generating 
melodies using a trained Transformer model. The class offers functionality to produce a
sequence of musical notes, starting from a given seed sequence and extending it to a
specified maximum length.

The MelodyGenerator class leverages the trained transformer model's ability to predict
subsequent notes in a melody based on the current sequence context. It acheives this by
iteratively appending each predicted note to the existing sequence and feeding this extended
sequence back into the model for further predictions.

This iterative process continues until the generated melody reaches the desired length or
an end-of-sequence token is predicted. The class utilizes a tokenizer to encode and 
decode note sequences to and from the format expected by the transformer model

Central Components:
- MelodyGenerator: The primary class defined in this script, responsible for the generation
    of melodies

Usage:
The MelodyGenerator class can be instantiated with a trained Transformer model and an
appropriate tokenizer. Once instantiated, it can generate meldoies by calling the
"generate" method with a starting note sequence.

Notes:
This class is intended to be used with a Transformer model that has been specifically
trained for melody generation tasks.
"""
...
class MidiGenerator:
    """
    Class to generate midi files using a trained Transformer model.

    This class encapsulates the inference logic for generating melodies based
    on a starting sequence
    """

    def __init__(self, transformer, tokenizer, max_length=50):
        """
        Initializes the MidiGenerator
    ...
    def _get_input_tensor(self, start_sequence):
        ...
        # self.tokenizer.fit_on_texts(start_sequence) # Need to fit on texts before calling texts to sequences
        # print("start_sequence fit on texts: ", start_sequence)
    ...
    def _decode_generated_sequence(self, generated_sequence):
        ...
        Parameters:
            generated_sequence (tf.Tensor): Tensor with note indices generated
